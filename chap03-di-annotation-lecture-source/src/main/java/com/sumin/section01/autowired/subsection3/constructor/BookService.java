...
public class BookService {
    ...
    /* 설명. 생성자 주입. 의존관계의 bean이 만들어져야 BookService가 동작. */
    /* 설명.
    ...
    *       (콩이 만들어질때부터 에러가 발생하기 때문에 개발이 용이. setter를 사용하게 되면, setter 사용시에 에러가 발생)
    *    3. field 주입 및 setter 주입의 단점을 보완
    *       (필드 주입은 간결하지만 남용할 수 있고 이후에 setter가 없이는 수정이 불가능하다.)
    *       (setter 주입은 불변 객체를 만들고자 함에 있어 문제가 발생할 수 있으며 객체의 변경이 필요할때만
    *        setter를 추가하는 것이 맞다 (가급적 변경의 여지를 남기지 않아야 된다)
    *    4. 테스트 코드 작성이 용이하다 (mock 객체 주입 용이)
    * */
    // 생성자 주입을 해야 final 필드값이 동작
    @Autowired
